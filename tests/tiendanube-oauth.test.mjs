...
// El diagnóstico público distingue la etapa D1 sin exponer la excepción interna.
test('OAuth start expone codigo seguro cuando falla el bootstrap D1', async () => {
  const rawSecret = 'SQL_PRIVATE_DETAIL_SHOULD_NOT_LEAK';
  const env = envFor(new MemoryD1());
  env.LEADS_DB = {
    prepare() {
  ...
  const html = await response.text();
  assert.match(html, /tiendanube_schema_bootstrap_failed/);
