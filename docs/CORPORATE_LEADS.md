...
La única persistencia corporativa sigue siendo Odoo. D1 pertenece a otros flujos y no se modifica. Se usa exclusivamente el campo HTML existente `crm.lead.description`, ya incluido en el payload original de corporate-leads y en `functions/api/leads.js`. Conserva la descripción comercial legible y agrega un bloque visible `<pre>` delimitado por `--- PRIMOFFICE CORPORATE DATA v1 ---` y `--- END PRIMOFFICE CORPORATE DATA ---`. El bloque se envía en el mismo `crm.lead.create` que los campos nativos: no hay una segunda escritura que pueda dejar el lead sin atribución.

El JSON contiene los ocho campos comerciales; `gclid`, `gbraid`, `wbraid`; `utm_source`, `utm_medium`, `utm_campaign`, `utm_term`, `utm_content`; `landing_url` y `referrer` iniciales; `captured_at` UTC generado por el servidor; `schema_version: 1`, `currency: "ARS"`, `status: "new"`, `estimated_value: null`, `quoted_value: null` y `final_sale_value: null`. Los valores son importes decimales en ARS; `null` significa desconocido y se distingue de cero. Parámetros opcionales ausentes se conservan como strings vacíos.
...
Publicar primero el backend compatible y después el frontend, en una etapa autorizada. No se necesita acceso al VPS ni modificación de esquema para implementar este cambio. El backend devuelve 503 si falta un campo nativo requerido o falla Odoo y no degrada silenciosamente la atribución. Esta tarea no publica ninguno de los dos componentes.
...
La protección contra doble click es por formulario cargado. Una respuesta de red perdida después de que Odoo haya creado el lead puede requerir revisión comercial antes de repetirlo; no se garantiza idempotencia entre requests independientes y no hay reintentos automáticos.
